...
export default {
  ...
  methods: {
    ...
    async _getTableData() {
      ...
      try {
        this.loading = true
        const { total, models } = await category.getPaginate(page, count, q)
    ...
    async onCellSave(data) {
      try {
        const res = await category.edit(data.id, data)
    ...
    onRemove(id) {
      this.$confirm('此操作将永久删除该信息, 是否继续?', '提示', {
        ...
        try {
          this.loading = true
          const res = await category.remove(id)
          this.$message.success(res.msg)
    ...
    async onHide(id) {
      try {
        this.loading = true
        const res = await category.hide(id)
        this.$message.success(res.msg)
        this._getTableData()
      } catch (err) {
        console.log(err)
    ...
    async onShow(id) {
      try {
        this.loading = true
        const res = await category.show(id)
        this.$message.success(res.msg)
        this._getTableData()
      } catch (err) {
        console.log(err)
