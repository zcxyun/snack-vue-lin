...
export default {
  ...
  methods: {
    ...
    async _getTableData() {
      ...
      try {
        this.loading = true
        const { total, models } = await theme.getPaginate(page, count, q)
    ...
    async onCellSave(data) {
      try {
        const res = await theme.edit(data.id, data)
    ...
    onRemove(id) {
      this.$confirm('此操作将永久删除该信息, 是否继续?', '提示', {
        ...
        try {
          this.loading = true
          const res = await theme.remove(id)
          this.$message.success(res.msg)
